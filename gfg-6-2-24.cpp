...
class Solution
{
    ...
    void solve(Node*root,int k,set<int>&s,int index,int val){
        if(root==NULL){
            return ;
        }
        ...
        solve(root->left,k,s,index+1,val);
        solve(root->right,k,s,index+1,val);
    }
    int printKDistantfromLeaf(Node* root, int k)
    {
    	...
    	set<int>s;
    	solve(root,k,s,0,root->data);
    	int leftAns=printKDistantfromLeaf(root->left,k);
    	int rightAns=printKDistantfromLeaf(root->right,k);
    	return s.size()+leftAns+rightAns;
    }
};
